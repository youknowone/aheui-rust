...
pub fn run_build(args: &BuildArgs) {
    ...
    match (args.codegen, args.emit) {
        ...
        (Codegen::C, Emit::Asm) => {
            print!("{}", crate::compile_to_c(&source));
        }
        ...
        (Codegen::Wasm32Wasi, Emit::Asm) => {
            print!("{}", crate::compile_to_wat_wasi(&source));
        }
        (Codegen::Wasm32Web, Emit::Asm) => {
            print!("{}", crate::compile_to_wat(&source));
        }
        (Codegen::Rust, Emit::Link) => {
            compile_rs(&generate_rs(&source, args.opt_level), &args.output_path());
        }
        (Codegen::C, Emit::Link) => {
            compile_c(&crate::compile_to_c(&source), &args.output_path());
        }
        ...
        (Codegen::Wasm32Wasi, Emit::Link) => {
            compile_wat(&crate::compile_to_wat_wasi(&source), &args.output_path());
        }
        (Codegen::Wasm32Web, Emit::Link) => {
            compile_wat(&crate::compile_to_wat(&source), &args.output_path());
        }
    }
}
...
fn run_cranelift(source: &str) {
    ...
    extern "C" fn sp_pop(_: *mut u8, _: usize) -> i64 {
        0
    }
    extern "C" fn sp_depth(_: *mut u8, _: usize) -> i64 {
        0
    }
