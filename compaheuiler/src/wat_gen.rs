...
fn emit_helpers(out: &mut String, has_queue: bool, has_port: bool, _has_io: bool) {
  ...
  ;; Safe division: returns 0 on div-by-zero, wraps INT64_MIN / -1
    ...
    (if (result i64) (i64.eqz (local.get $b))
      ...
      (else
        (if (result i64) (i32.and
          ...
          (else (i64.div_s (local.get $a) (local.get $b)))
        )
      )
    )
  )

  ;; Safe remainder: returns 0 on div-by-zero, 0 for INT64_MIN % -1
    ...
    (if (result i64) (i64.eqz (local.get $b))
      ...
      (else
        (if (result i64) (i32.and
            ...
            (i64.eq (local.get $b) (i64.const -1)))
          (then (i64.const 0))
          (else (i64.rem_s (local.get $a) (local.get $b)))
        )
      )
    )
  )
